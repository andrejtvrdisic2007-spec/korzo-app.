...
menu_database = {
    ...
    "Sir 100g": {"category": "Hladno predjelo", "price": 340.00, "calories": 350, "protein": 20, "carbs": 2, "fats": 28, "allergens": "Mleko", "dietary_tags": ["Vegetarijansko"], "description": "Domaći beli sir", "image": "slike/sir.jpg"},
    ...
    "Kupus salata 300g": {"category": "Salate", "price": 330.00, "calories": 80, "protein": 2, "carbs": 10, "fats": 4, "allergens": "Nema", "dietary_tags": ["Vegansko", "Vegetarijansko"], "description": "Sveža kupus salata", "image": "slike/kupus_salata.jpg"},
    "Ljuta paprika у ulju 1 komad": {"category": "Salate", "price": 150.00, "calories": 50, "protein": 0, "carbs": 2, "fats": 4, "allergens": "Nema", "dietary_tags": ["Vegansko", "Vegetarijansko"], "description": "Pečena ljuta paprika", "image": "slike/ljuta_paprika.jpg"},
    "Lepinja 1 komad": {"category": "Dodaci", "price": 120.00, "calories": 250, "protein": 7, "carbs": 50, "fats": 2, "allergens": "Gluten", "dietary_tags": ["Vegansko", "Vegetarijansko"], "description": "Sveža domaća lepinja", "image": "slike/lepinja.jpg"},
    ...
    "Kugla kajmaka 1 komad": {"category": "Dodaci", "price": 180.00, "calories": 200, "protein": 2, "carbs": 1, "fats": 22, "allergens": "Mleko", "dietary_tags": ["Vegetarijansko"], "description": "Domaći zreli kajmak", "image": "slike/kugla_kajmaka.jpg"}
}
...
def prikazi_sliku(putanja): return putanja if os.path.exists(putanja) else "https://via.placeholder.com/400x250.png?text=Slika+uskoro"
...
def prikazi_konobara():
    ...
    baza = ucitaj_iz_baze()
    if not baza: st.success("Тренутно нема активних поруџбина.")
    else:
        cols = st.columns(3)
        for i, (sto, podaci) in enumerate(baza.items()):
            with cols[i % 3]:
                with st.container(border=True):
                    if podaci['zove_konobara']: st.error(f"🔔 СТО {sto} ВАС ЗОВЕ!")
                    elif podaci['trazi_racun']: st.warning(f"💳 СТО {sto} ТРАЖИ РАЧУН ({podaci['nacin_placanja']})")
...
def prikazi_gosta(sto):
    ...
    for jelo, qty in moj_sto["stavke"].items():
        ...
        ukupno += cena
        st.sidebar.write(f"**{qty}x** {jelo} ({cena:.2f} RSD)")
    st.sidebar.metric("Укупно за наплату:", f"{ukupno:.2f} RSD")
    ...
    # Категорије
    kategorije = list(set([info["category"] for info in menu_database.values()]))
            ...
            for j, (ime, info) in enumerate(items.items()):
                with cols[j % 2]:
                    with st.container(border=True):
                        ...
                        if st.button(f"🛒 Додај у поруџбину", key=f"add_{ime}"):
    ...
    upit = st.chat_input("Питајте нешто о храни (нпр. шта има највише протеина?)...")
    if upit:
        with st.chat_message("user"): st.markdown(upit)
        # Хранимо AI подацима из твог менија
        context = f"Ти си конобар у ресторану Корзо. Ево нашег менија: {list(menu_database.keys())}. Питање госта: {upit}"
